...
import com.rratchet.android.compress.app.utils.FileUtils;
import com.rratchet.android.compress.app.utils.ToastUtils;
                ...
                public void run() {
                    ...
                    try {
                        if (requestCode == FILE_SELECT_CODE_UNZIP) {
                            ...
                            ToastUtils.show("解压文件目录：" + output, true);
                        } else {
                            name += ".rar";
                            ToastUtils.show("压缩文件目录：" + CompressHelper.compressRarFile(path, new File(getExternalCacheDir(), name).getAbsolutePath()), true);
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
